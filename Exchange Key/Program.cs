...
namespace ExchangeKey
{
    ...
    {
        static int sz = 100;
        static List<bool> parr = new List<bool>(new bool[sz + 1]);
        static void sieve()
        {
        ...
        static void ExchangeKey(int p, int g, int xa, int xb)
        {
            ...
            if (k1 == k2)
            {
                Console.WriteLine("Chosen Prime number is= " + p);
                Console.WriteLine("Chosen Primitive root number is= " + g);
                Console.WriteLine("Chosen Private keys are Xa= " + xa + " Xb= " + xb);
